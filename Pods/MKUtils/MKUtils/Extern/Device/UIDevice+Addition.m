...
+ (NSString *)batteryPercent {
    ...
    NSString *result = MKString(@"%.2f",deviceLevel);
    return result;
}
...
+ (NSString *)usedCPUPercent {
    ...
    NSString *result = MKString(@"%.2f",_usedCPUPercent);
    return result;
}
...
+ (NSString *)freeMemoryPercent {
    ...
    NSString *result = MKString(@"%.2f",_freeMemory / (_freeMemory + _usedMemory));
    return result;
}
...
+ (NSString *)machinePhoneName {
    ...
    dispatch_once(&one, ^{
        ...
        NSDictionary *keyValue = @{
                                   ...
                                   @"iPad7,1" : @"iPad Pro (12.9-inch 2rd)",
                                   @"iPad7,2" : @"iPad Pro (12.9-inch 2rd)",
        ...
        phone_name = [NSString stringValue:keyValue[deviceName]];
    });
    return phone_name;
}
...
+ (NSString *)deviceName {
    ...
    return [NSString stringValue:machineName];
}
...
+ (NSString *)UUID {
    ...
    CFRelease(string);
    return [NSString stringValue:result];
}

#pragma mark 获取IDFA
+ (NSString *)IDFA {
    NSString *adId = [[[ASIdentifierManager sharedManager] advertisingIdentifier] UUIDString];
    return [NSString stringValue:adId];
}
...
+ (NSString *)IDFV {
    if ([[UIDevice currentDevice] respondsToSelector:@selector(identifierForVendor)]) {
        ...
        return [NSString stringValue:UUIDString];
    }
    return @"";
}
...
+ (NSString *)macAddress {
    ...
    free(buf);
    return [NSString stringValue:[outString uppercaseString]];
}
